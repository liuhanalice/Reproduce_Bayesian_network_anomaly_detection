...
# We generate the simulation point clouds
def depression_circle_v2(options, num_p=150):
    bg_k = options.bg_k
    bg_size = options.bg_size
    bg_std_depth = options.bg_std_depth
    bg_std_xy = options.bg_std_xy
    outliers_rate = options.outliers_rate
    outliers_std_depth = options.outliers_std_depth
    outliers_std_xy = options.outliers_std_xy
    defect_depth = options.defect_depth
    defect_radius = options.defect_radius
    spline_flag = options.spline_flag
    # numpy_array 2rd shape is the number of defects
    defect_pos = options.defect_pos * (bg_size / 2)
    defect_trans = options.defect_trans
    n_knot = options.spline_knot
    ...
    if spline_flag == True:
        ...
        spline_paras = options.spline_paras
    ...
    for i in range(num_p):
        for j in range(num_p):
            ...
            if min_dist <= defect_radius * 1.15 and min_dist >= defect_radius * 0.85:
                add = np.array([[0., 0.], [2 * bg_size / 1.5 / num_p / 2, 2 * bg_size / num_p / 2],
                                [2 * bg_size / 1.5 / num_p / 2, -2 * bg_size / num_p / 2]])

                for n in range(2):
                    if n == 1:
                        if np.random.uniform(0, 1) >= 0.5:
                            n = 2
                    new_coo_x = coo_x + add[n, 0]
                    new_coo_y = coo_y + add[n, 1]
                    coo = np.array([[new_coo_x], [new_coo_y]], np.float64)
                    dist = np.linalg.norm(coo - defect_pos, axis=0)
                    min_dist = np.min(dist)
                    if spline_flag:
                        cur_point, cur_label, cur_surface, cur_delta_z = single_point_SPLINE(new_coo_x, new_coo_y,
                                                                                             min_dist,
                                                                                             bg_std_xy, bg_std_depth,
                                                                                             bg_k,
                                                                                             outliers_std_xy,
                                                                                             outliers_rate,
                                                                                             outliers_std_depth,
                        ...
                        cur_point, cur_label, cur_surface, cur_delta_z = single_point_V3(new_coo_x, new_coo_y, min_dist,
                                                                                         bg_std_xy,
                                                                                         bg_std_depth, bg_k,
                                                                                         outliers_std_xy,
                                                                                         outliers_rate,
                                                                                         outliers_std_depth,
                                                                                         defect_trans, defect_radius,
                                                                                         defect_depth, p_dist,
                                                                                         options.step)
                ...
                if spline_flag:
                    cur_point, cur_label, cur_surface, cur_delta_z = single_point_SPLINE(coo_x, coo_y, min_dist,
                                                                                         bg_std_xy, bg_std_depth, bg_k,
                                                                                         outliers_std_xy,
                                                                                         outliers_rate,
                                                                                         outliers_std_depth,
                                                                                         defect_trans, defect_radius,
                                                                                         defect_depth, p_dist,
                                                                                         options.step,
                                                                                         bg_delta_z[i * num_p + j, 0])
                else:
                    cur_point, cur_label, cur_surface, cur_delta_z = single_point_V3(coo_x, coo_y, min_dist, bg_std_xy,
                                                                                     bg_std_depth, bg_k,
                                                                                     outliers_std_xy,
                                                                                     outliers_rate, outliers_std_depth,
                                                                                     defect_trans, defect_radius,
                                                                                     defect_depth, p_dist, options.step)
                pts.append(cur_point)
                label.append(cur_label)
                surface_index.append(cur_surface)
                delta_z.append(cur_delta_z)
...
# This is a polynomial kind of smooth surface
def single_point_V3(coo_x, coo_y, min_dist, bg_std_xy, bg_std_depth, bg_k, outliers_std_xy,
                    outliers_rate, outliers_std_depth, defect_trans, defect_radius, defect_depth, p_dist, step):
    # in this case, defect trans means the size of transition half-sphere
    # transition_depth = -defect_depth + bg_k * 10 * (defect_trans * min_dist) ** 2
    ...
    if min_dist >= defect_radius * defect_trans:
        ...
        label = 1
        surface = 2
    if rand_index >= 1 - outliers_rate:
        outliers_noise_z = np.random.normal(scale=outliers_std_depth)
        outlier_noise = np.array(
            [[np.random.normal(scale=outliers_std_xy)], [np.random.normal(outliers_std_xy)],
             [outliers_noise_z + np.sign(outliers_noise_z) * (np.random.uniform(1, 5 * bg_std_depth + 1))]])
        point += outlier_noise
        label = 2
    return point, label, surface, delta_z


# This is a B-spline kind of smooth surface
def single_point_SPLINE(coo_x, coo_y, min_dist, bg_std_xy, bg_std_depth, bg_k, outliers_std_xy,
                        outliers_rate, outliers_std_depth, defect_trans, defect_radius, defect_depth, p_dist, step,
                        bg_delta):
    transition_depth = -defect_depth
    rand_index = np.random.uniform(0, 1)
