...
def Experiments_of_Noise_Level(noise):
    ...
    for i in range(num_experiments):
        ...
        pcd = o3d.io.read_point_cloud('../EM_DRG_result/Noise_0p15/data3/pcd_' + str(i) + '.pcd')
        label = np.load('../EM_DRG_result/Noise_0p15/data3/label_' + str(i) + '.npy')
        est_label = window_plane_V1(pcd, label)
        np.save('../EM_DRG_result/Noise_0p15/result3/IST_OPTIC_est_label_' + str(i), est_label)
    ...
    np.save('./EM_DRG_result/Noise_0p15/result3/IST_OPTIC_FPR', FPR_vec)
    np.save('./EM_DRG_result/Noise_0p15/result3/IST_OPTIC_FNR', FNR_vec)
