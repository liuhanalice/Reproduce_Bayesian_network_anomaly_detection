...
def experiments_depth(depth):
    ...
    for i in range(num_experiments):
        ...
        o3d.io.write_point_cloud('./Replicate_EM_DRG_result/Depth_7/data3/pcd_' + str(i) + '.pcd', pcd)
        np.save('./Replicate_EM_DRG_result/Depth_7/data3/label_' + str(i), label)

        est_label = EM_Directed_Graphical_Model(pcd, label, NN=35, n_knot=10, epoch_EM=16, C_f=1 / 4,
                                                C_ns=1 / 48)

        np.save('./Replicate_EM_DRG_result/Depth_7/result3/EM_DRG_est_label_' + str(i), est_label)
    ...
    np.save('./Replicate_EM_DRG_result/Depth_7/result3/EM_DRG_FPR', FPR_vec)
    np.save('./Replicate_EM_DRG_result/Depth_7/result3/EM_DRG_FNR', FNR_vec)
