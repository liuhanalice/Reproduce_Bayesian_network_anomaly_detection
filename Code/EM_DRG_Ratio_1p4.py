...
def experiments_ratio(ratio):
    print("Experiments of Ratio Range (EM_DIRECTED_GRAPH)!")
    print("Ratio:", ratio)
    num_experiments = 30
    ...
    for i in range(num_experiments):
        ...
        options = EM_options(0.0004, 0.01, 5, 4, ratio, cur_pos, bg_std_depth=0.10, step=-0.35, spline_flag=False)
        print("options", options.bg_k, options.bg_std_depth)
        pcd, label, _, _ = depression_circle_v2(options, num_p=150)

        o3d.io.write_point_cloud('./EM_DRG_result/Ratio_1p4/data3/pcd_' + str(i) + '.pcd', pcd)
        np.save('./EM_DRG_result/Ratio_1p4/data3/label_' + str(i), label)

        est_label = EM_Directed_Graphical_Model(pcd, label, NN=35, n_knot=10, epoch_EM=16, C_f=1 / 4,
                                                C_ns=1 / 48)

        np.save('./EM_DRG_result/Ratio_1p4/result3/EM_DRG_est_label_' + str(i), est_label)
        ...
        FPR_vec[i] = FPR
        FNR_vec[i] = FNR
    print("FPR_vec_Ratio" + str(ratio), FPR_vec)

    print("FNR_vec_Ratio" + str(ratio), FNR_vec)

    np.save('./EM_DRG_result/Ratio_1p4/result3/EM_DRG_FPR', FPR_vec)
    np.save('./EM_DRG_result/Ratio_1p4/result3/EM_DRG_FNR', FNR_vec)
